		...
		evacuatingNode = new BehaviourNode(){
			...
			public double next(Steppable s, double time) {
				...
				if(p.evacuatingTime < 0) {
					...
					Shelter myShelter = p.selectTargetShelter();
					p.headForShelter(myShelter);
					
					if(p.dependentOf != null)
						p.dependentOf.headForShelter(myShelter);
					return 1;
				}
		...
		travelToHomeShelteringNode = new BehaviourNode() {
			...
			public double next(Steppable s, double time) {
				...
				if(outcome > 0 && p.finishedPath()){
					...
					Shelter shelter = p.selectTargetShelter();
